...
<div class="notes form large-9 medium-8 columns content">
    ...
    <fieldset>
        ...
        <?php
            ...
            echo $this->Form->control('bullets');
...
<div class="notes form large-9 medium-8 columns content">
    <?= $this->Form->create($note) ?>
    <fieldset>
        <legend class="mb-4"><?= __('Editar nota') ?></legend>

        <label for="date">Fecha de la Nota</label>
        <input name="date" type='text' class='form-control datepicker-here' data-language='en' data-timepicker="false" data-date-format="yyyy-m-d" value="<?php if(isset($note->date)) { echo ($note->date->format('Y-m-d')); } ?>"/>
